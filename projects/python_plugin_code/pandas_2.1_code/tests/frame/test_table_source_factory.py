...
def test_create_for_dict_orient_tight():
    ...
    assert table_frame.column_labels == [['x', '1'], ['y', '2']]
    assert table_frame.index_labels == [['a', 'b'], ['a', 'c']]
    assert table_frame.legend.index == ['n1', 'n2']
    assert table_frame.legend.column == ['z1', 'z2']
    assert list(map(lambda c: c.value, chain(*table_frame.cells))) == ['1', '3', '2', '4']
