...
class PatchedStylerContext(PandasTableSourceContext):
    ...
    def _get_frame_column_info(self) -> TableStructureColumnInfo:
        ...
        index_legend = [
            ...
            for lvl, lbl in enumerate(self._visible_frame.index_names)
        ...
        column_legend = [
            self._formatter.format_index(lbl)
            for lvl, lbl in enumerate(self._visible_frame.column_names)
            # hide_columns_ := list of booleans, True means lbl should be excluded
    ...
    @staticmethod
    def __get_patcher_for_supported_pandas_style_functions(org_frame: DataFrame, todo: StylerTodo) -> Optional[
        TodoPatcher]:
