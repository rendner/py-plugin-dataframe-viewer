...
class TableFrameGenerator(AbstractTableFrameGenerator):
    ...
    def generate(self,
        ...
        dict_props = chunk.to_dict(orient="tight")
        ...
        legend_label = None if exclude_col_header and exclude_row_header else self._extract_legend_label(dict_props,
                                                                                                         formatter)
    ...
    @staticmethod
    def _extract_legend_label(dict_props: dict, formatter: ValueFormatter) -> TableFrameLegend:
        index_legend = [formatter.format_index(n) for n in dict_props.get("index_names", []) if n is not None]
        column_legend = [formatter.format_index(n) for n in dict_props.get("column_names", []) if n is not None]
