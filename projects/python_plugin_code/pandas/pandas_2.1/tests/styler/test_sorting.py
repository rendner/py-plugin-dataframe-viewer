...
def test_sort_values_before_styling_breaks_styling():
    ...
    assert "background-color: yellow;" in styler_asc.to_html()
    ...
    assert "background-color: yellow;" not in styler_asc.to_html()
...
def test_hide_row_of_a_styled_subset_after_styling_should_not_break_sorting():
    assert_patched_styler_sorting(
        df,
        lambda styler: styler.highlight_max(subset=IndexSlice[2:4]).hide(axis="index", subset=IndexSlice[3:3]),
        2,
        2,
        [0],
        [False],
        lambda styler: styler.hide(axis="index", subset=IndexSlice[3:3]),
    )


def test_hide_row_of_a_styled_subset_before_styling_should_not_break_sorting():
    assert_patched_styler_sorting(
        df,
        lambda styler: styler.hide(axis="index", subset=IndexSlice[3:3]).highlight_max(subset=IndexSlice[2:4]),
        2,
        2,
        [0],
        [False],
