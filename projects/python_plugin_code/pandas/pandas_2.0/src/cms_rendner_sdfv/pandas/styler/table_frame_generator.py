...
class TableFrameGenerator(AbstractTableFrameGenerator):
    ...
    def _extract_legend_label(self, formatter: ValueFormatter, chunk: StyledChunk) -> Union[None, TableFrameLegend]:
        hidden_col_lvl_map = chunk.hidden_col_level_map
        hidden_row_lvl_map = chunk.hidden_row_level_map
        index_legend = [
            formatter.format_index(n)
            for i, n in enumerate(self._visible_frame.index_names)
            if n is not None and not hidden_row_lvl_map[i]
        ]

        if not index_legend and any(hidden_row_lvl_map):
        ...
        column_legend = [
            ...
            if n is not None and not hidden_col_lvl_map[i]
        ]
