...
class Chunk:
    ...
    def create_cell_iloc_into_org_frame_translator(self) -> Callable[[Tuple[int, int]], Tuple[int, int]]:
        ri_translator = self.create_row_into_org_frame_translator()
        ci_translator = self.create_col_into_org_frame_translator()

        def translate(k: Tuple[int, int]) -> Tuple[int, int]:
            return ri_translator(k[0]), ci_translator(k[1])

        return translate

    def create_col_into_org_frame_translator(self) -> Callable[[int], int]:
        r = self.region
        f = self._frame

        def translate(k: int):
            return f.i_cols[r.first_col + k]

        return translate

    def create_row_into_org_frame_translator(self) -> Callable[[int], int]:
        r = self.region
        f = self._frame

        def translate(k: int):
            return f.i_rows[r.first_row + k]
