...
def create_validator(style: Styler) -> HTMLPropsValidator:
    return HTMLPropsValidator(PatchedStylerContext(style))
...
def test_multi_index_df_without_styles(rows_per_chunk: int, cols_per_chunk: int):
    styler = mi_df.style
    result = create_validator(styler).test_only_validate(rows_per_chunk, cols_per_chunk)
...
def test_hide_index(subset, rows_per_chunk: int, cols_per_chunk: int):
    styler = df.style.hide_index(subset=subset)
    result = create_validator(styler).test_only_validate(rows_per_chunk, cols_per_chunk)
    assert result.actual == result.expected
    assert result.is_equal is True
...
def test_too_large_parameters():
    styler = df.style
    result = create_validator(styler).test_only_validate(len(df.index) * 2, len(df.columns) * 2)
    assert result.actual == result.expected
    assert result.is_equal is True
...
def test_multiple_style_functions_with_subset():
    ...
    result = create_validator(styler).test_only_validate(2, 2)
    assert result.actual == result.expected
    assert result.is_equal is True
...
def test_non_string_values():
    ...
    styler = my_df.style
    result = create_validator(styler).test_only_validate(1, 1)
    assert result.actual == result.expected
    assert result.is_equal is True
