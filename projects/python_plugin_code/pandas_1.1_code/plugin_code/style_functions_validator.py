...
class StyleFunctionsValidator:
    ...
    def __count_apply_calls(self) -> int:
        if len(self.__styler._todo) == 0:
            return 0
        return len([not StylerTodo.is_applymap_tuple(t) for t in self.__styler._todo])
